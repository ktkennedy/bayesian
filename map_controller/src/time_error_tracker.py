...
class TimeErrorTracker:
    def __init__(self):
        ...
        self.loop_rate = 1000  # rate in hertz
        ...
        waypoint_file = rospy.get_param('~waypoint_file', '/home/kennedy/f1tenth_ws/src/MAP-Controller/F110_ROS_Simulator/maps/u/optimized_traj.txt')
        self.load_waypoints_from_file(waypoint_file)
        self.waypoint_times = [None]*len(self.waypoints)
    
    def load_waypoints_from_file(self, filepath):
       with open(filepath, 'r') as file:
            lines = file.readlines()
            self.waypoints = np.array([list(map(float, line.strip().split(','))) for line in lines])
    ...
    def waypoint_cb(self, data):
        ...
        for i, waypoint in enumerate(data.wpnts):
            ...
            if self.distance(self.position[:2], [waypoint.x_m, waypoint.y_m]) < 0.5:
                self.waypoint_times[i] = time.time() - self.start_time
                print('Waypoint %d: Passed at time %.3f' % (i, self.waypoint_times[i]))  # print the time when a waypoint is passed
    def tracking_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.distance(self.waypoints[idx_nearest_waypoint, :2], self.waypoints[0, :2]) < 0.5:
                if lap_count > 0 and time.time() - lap_timer > 1:
                    ...
                    lap_count = 1
                lap_timer = time.time()
            if None not in self.waypoint_times:  # check if all waypoints have been passed
                for i, time_passed in enumerate(self.waypoint_times):
                    print('Waypoint %d: Passed at time %.3f' % (i, time_passed))
                break  # exit the loop after printing the times
            # record the time when a waypoint is passed
            # if self.distance(self.position[:2], self.waypoints[idx_nearest_waypoint, :2]) < 0.01:
            #     if idx_nearest_waypoint not in waypoint_times:
            #         waypoint_times[idx_nearest_waypoint] = time.time()-self.start_time
            # # print the times when each waypoint was passed
            # for idx, time_passed in waypoint_times.items():
            #     print('Waypoint %d: Passed at time %.3f' % (idx, time_passed)) 
