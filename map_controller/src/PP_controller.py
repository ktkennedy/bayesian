...
class Controller:
    def __init__(self):
        ...
        self.param_q_pp = rospy.get_param('~q_pp')
        self.param_m_pp = rospy.get_param('~m_pp')
        self.param_t_clip_min = rospy.get_param('~t_clip_min')
        self.param_t_clip_max = rospy.get_param('~t_clip_max')
        ...
        self.waypoints_pub = rospy.Publisher('/published_waypoints', MarkerArray, queue_size=10)
        ...
        #     self.position = self.waypoints[0, :3]
        # else:
        #     self.position = None
    ...
    def load_waypoints_from_file(self, filepath):
        with open(filepath, 'r') as file:
            ...
            self.waypoints = np.array([list(map(float, line.strip().split(','))) 
            for line in lines])
    ...
    def control_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.waypoints.shape[0] > 2:
                ...
                target_speed = self.waypoints[idx_nearest_waypoint, 3]
                ...
                if lookahead_point.any() is not None:
                    ...
                    self.visualize_waypoint(self.waypoints)
    ...
    def map_to_baselink(self, point):
        ...
        pose_in_baselink = self.tf_listener.transformPose("base_link", posestamped_in_map)
    ...
    def visualize_waypoint(self, waypoints):
        """
        Publishes waypoints as markers for visualization in RViz.

        Args:
            waypoints (numpy.ndarray): An array of waypoints to visualize.
        """
        marker_array = MarkerArray()
        for i, waypoint in enumerate(waypoints):
            marker = Marker()
            marker.header.frame_id = "map"
            marker.header.stamp = self.ros_time.now()
            marker.id = i
        ...
        # rospy.loginfo("Published waypoints for visualization")   
